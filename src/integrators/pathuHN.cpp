...
class UniformHemisphereNoisePathIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    UniformHemisphereNoisePathIntegrator(const Properties &props) : Base(props) {
        // 1. 读取噪声强度 (Sigma)
        ...
        dr::tie(ls) = dr::while_loop(
            ...
            [this, scene](LoopState &ls) {
                ...
                if (dr::any_or<true>(is_miss)) {
                    ...
                    // 2. 获取场景的环境光 (EnvMap)
                    EmitterPtr env = scene->environment();
                    Mask has_env = is_miss && (env != nullptr);
                    
                    // 初始底色设置为 XML 中读取的 m_bg_color
                    Spectrum base_bg = m_bg_color;

                    // 3. 如果场景有 EnvMap，读取 HDRI 像素值覆盖底色
                    if (dr::any_or<true>(has_env)) {
                        // env->eval 会根据当前的表面交互点 (si) 的光线方向提取贴图颜色
                        Spectrum env_Le = env->eval(si, has_env);
                        dr::masked(base_bg, has_env) = env_Le;
                        
                        // 有环境光的地方也是有效像素（防透明全黑）
                        ls.valid_ray |= has_env; 
                    }

                    // 4. 计算最终背景颜色: (EnvMap 或 纯色) + Sigma * Noise
                    Spectrum bg = base_bg + Spectrum(m_bg_sigma) * noise_val;
    ...
    std::string to_string() const override {
        return tfm::format("UniformHemisphereNoisePathIntegrator[\n"
                           "  max_depth = %u,\n"
                           "  rr_depth = %u,\n"
                           "  bg_mean = %s,\n"
                           "  bg_sigma = %f\n"
    ...
    MI_DECLARE_CLASS(UniformHemisphereNoisePathIntegrator)
};

MI_EXPORT_PLUGIN(UniformHemisphereNoisePathIntegrator)
